...
public class TileEntityJarRenderer extends TileEntitySpecialRenderer<TileEntityJar> {
    @Override
    public void render(TileEntityJar tile, double x, double y, double z, float partialTicks, int destroyStage, float alpha) {
        ...
        double min = 5.5D / 16.0D;
        double max = 10.5D / 16.0D;
        double yMin = 0.5D / 16.0D;
        double yMax = yMin + tile.getFluidFullness() * (6.0D / 16.0D);
        ...
        face(buffer, min, max, yMin, yMax, sprite, r, g, b, a);
        tessellator.draw();
    ...
    private static void face(BufferBuilder buffer, double min, double max, double yMin, double yMax,
        ...
        buffer.pos(max, yMax, min).tex(sprite.getMaxU(), sprite.getMinV()).color(r, g, b, a).normal(1.0F, 0.0F, 0.0F).endVertex();
        buffer.pos(max, yMin, min).tex(sprite.getMaxU(), sprite.getMaxV()).color(r, g, b, a).normal(1.0F, 0.0F, 0.0F).endVertex();
    }
}
